...
def compareScreenshot(compImage, refImage, id):
    with Image(filename=refImage) as base:
        with Image(filename=compImage) as img:
            ...
            print(result_metric, ' -- 1.0 if reference image and comparison image match. If the images do not match see https://github.com/PmaFynn/BA_v1/blob/CiServerImages/src/compImages/ for the resulting difference image! ' )
